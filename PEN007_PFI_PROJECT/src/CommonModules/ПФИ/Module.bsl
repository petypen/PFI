...
#Область СозданиеРеквизитовИКоманд
...
// Параметры:
//	Форма - ФормаКлиентскогоПриложения - Форма в которой создаются реквизиты
//	ИмяТаблицы - Строка - Имя таблицы куда добавляются реквизиты
//  стРеквизиты - Структура - Описание реквизитов:
//  	* ключ - Строка - Имя реквизита
//  	* значение - ОписаниеТипов - описание типа реквизита
//
Процедура СоздатьРеквизитыТаблицы(Форма, ИмяТаблицы, стРеквизиты) Экспорт

	ДобавляемыеРеквизиты = Новый Массив;
	Для Каждого элементСтруктуры Из стРеквизиты Цикл
		НовыйРеквизит = Новый РеквизитФормы(элементСтруктуры.Ключ, элементСтруктуры.Значение, ИмяТаблицы);
		ДобавляемыеРеквизиты.Добавить(НовыйРеквизит);
	КонецЦикла;
...
//    Ключ - Строка - Имя колонки и одновременно часть пути к данным колонки, например:
//      Путь к данным = <Имя списка> . <Имя колонки>
//    Значение - Строка - Заголовок колонки таблицы формы
...
// Горизонтальное положение элемента.
// 
// Параметры:
//  Положение - Число - код положения элемента, где значение:
//    1 - Лево
//    2 - Центр
//    3 - Право
//    0 - Авто
// 
// Возвращаемое значение:
//  ГоризонтальноеПоложениеЭлемента - положение элемента формы по горизонтали
Функция ГоризонтальноеПоложениеЭлемента(Положение)

	ПоложениеЭлемента = ГоризонтальноеПоложениеЭлемента.Авто; // ГоризонтальноеПоложениеЭлемента -
...
Функция ГруппаОбычная(Форма, ИмяГруппы, РодительГруппы, Заголовок = "", ГруппировкаОбычнойГруппы = 0,
...
//  ГруппаФормы - Новая группа формы.
//@skip-check method-too-many-params
Функция ГруппаОбычнаяБезОтображения(Форма, ИмяГруппы, РодительГруппы, ГруппировкаОбычнойГруппы = 0,
	РастягиватьГоризонтально = 0, РастягиватьВертикально = 0, стСвойстваЭлемента = Неопределено,
	ЭлементПеред = Неопределено) Экспорт
...
//@skip-check method-too-many-params
Функция СтраницаГруппыСтраниц(Форма, ИмяСтраницы, ГруппаСтраниц, Заголовок = "", ГруппировкаОбычнойГруппы = 0,
	РастягиватьГоризонтально = 0, РастягиватьВертикально = 0, стСвойстваЭлемента = Неопределено,
...
Функция ПолеВыпадающийСписок(Форма, ИмяСписка, РодительСписка, Заголовок = "", ЗначенияВыбора,
	...
	Для Каждого элементМассива Из ЗначенияВыбора Цикл
		НовыйЭлемент.СписокВыбора.Добавить(элементМассива);
	КонецЦикла;
...
Функция ПолеПереключатель(Форма, ИмяПереключателя, РодительЭлемента, Заголовок = "", ЗначенияПереключателя,
	КодВидаПереключателя, стСвойстваЭлемента = Неопределено, стСобытияЭлемента = Неопределено, ЭлементПеред = Неопределено) Экспорт

	СоздатьРеквизит(Форма, ИмяПереключателя, ОписаниеТипаЧисло(10, 0));
	НовыйЭлемент = Поле(Форма, ИмяПереключателя, РодительЭлемента, Заголовок, 4, ИмяПереключателя,
		стСвойстваЭлемента, стСобытияЭлемента, ЭлементПеред);
...
// Возвращаемое значение:
//  ДекорацияФормы - Новая декорация формы
Функция ДекорацияНадпись(Форма, ИмяДекорации, РодительДекорации, Заголовок,
...
//  стКолоноки - Структура - Колонки новой таблицы, где
...
Функция ТаблицаФормы(Форма, ИмяТаблицы, РодительТаблицы, ПутьКДанным, стКолоноки = Неопределено,
	стСвойстваЭлемента = Неопределено, стСобытияЭлемента = Неопределено, СвояКоманднаяПанель = Ложь,
	ЭлементПеред = Неопределено) Экспорт

	ЕслиСтруктураНеопределеноТоНовая(стКолоноки);
	ЕслиСтруктураНеопределеноТоНовая(стСвойстваЭлемента);
	ЕслиСтруктураНеопределеноТоНовая(стСобытияЭлемента);

	Если СвояКоманднаяПанель Тогда
		ГруппаДинамическогоСписка = ГруппаОбычнаяБезОтображения(Форма, "ДополнительнаяГруппаКоманднаяПанель" + ИмяТаблицы,
	...
	СоздатьКолонкиТаблицыФормы(Форма, ТаблицаФормы, стКолоноки);
...
Функция КолонкаТаблицыФормы(Форма, ИмяКолонки, Родитель, ТипКолонки = 0, Заголовок = "", ПутьКДанным,
	ОтображатьШапку = Истина, ОтображатьПодвал = Ложь, стСвойстваЭлемента = Неопределено, стСобытияЭлемента = Неопределено,
	ЭлементПеред = Неопределено) Экспорт
...
// 
// Возвращаемое значение:
//  ГруппаФормы - Группа колонок таблицы формы
...
Функция СтруктураСвойствРасположение(Ширина = Неопределено, Высота = Неопределено, АвтоМаксимальнаяШирина = Неопределено,
	...
	Если тГоризонтальноеПоложение <> Неопределено Тогда
		СтруктураСвойств.Вставить("ГоризонтальноеПоложение", ГоризонтальноеПоложениеЭлемента(
			тГоризонтальноеПоложение));
	КонецЕсли
	;
	Если тГоризонтальноеПоложениеВШапке <> Неопределено Тогда
		СтруктураСвойств.Вставить("ГоризонтальноеПоложениеВШапке", ГоризонтальноеПоложениеЭлемента(
			тГоризонтальноеПоложениеВШапке));
	КонецЕсли
	;
	Если тГоризонтальноеПоложениеВПодвале <> Неопределено Тогда
		СтруктураСвойств.Вставить("ГоризонтальноеПоложениеВПодвале", ГоризонтальноеПоложениеЭлемента(
